...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(_.toLower).toList.map(
    _ match {
      case (a, b) => (a, b.length)
    }).sortWith(_._1 < _._1)
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    (for (c_s <- s.foldLeft("")(_ + _).groupBy(_.toLower))
      yield (c_s._1, c_s._2.length)).toList.sortWith(_._1 < _._1)
  }
  ...
  def occurrenceAnagrams(occ: Occurrences): List[Sentence] = occ match {
    case Nil => List(Nil)
    case _ => {
      val combs = combinations(occ)
      for (i <- combs if dictionaryByOccurrences.keySet(i);
           j <- dictionaryByOccurrences(i);
           s <- occurrenceAnagrams(subtract(occ, i))) yield {
        j :: s
      }
    }
  }
